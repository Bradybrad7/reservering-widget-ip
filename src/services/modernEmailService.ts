 ...
 * 🎭 Modern Email System
 * 
 * Dit is het NIEUWE email systeem met master template.
 * Gebruikt de "Dark Theatre" branding met table-based layouts.
 * 
 * Voor gebruik:
 * 1. Importeer: import { modernEmailService } from './modernEmailService';
 * 2. Gebruik: await modernEmailService.sendByStatus(reservation, event);
 * 3. Preview: const html = modernEmailService.previewHTML(reservation, event);
 * 
 * ⚠️ LET OP: Dit vervangt geleidelijk de oude emailService functies
 */
 ...
 */
export const modernEmailService = {
  /**
