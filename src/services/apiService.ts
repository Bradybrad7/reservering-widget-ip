...
export const apiService = {
  ...
  async submitReservation(formData: CustomerFormData, eventId: string): Promise<ApiResponse<Reservation>> {
    ...
    try {
      ...
      // 🚫 NEW: Check if customer is blocked (November 2025)
      const { noShowService } = await import('./noShowService');
      const isBlocked = await noShowService.isCustomerBlocked(formData.email);
      
      if (isBlocked) {
        return {
          success: false,
          error: 'Helaas kunt u momenteel geen boekingen plaatsen. Neem contact op met het theater voor meer informatie.'
        };
      }
      ...
      const reservationData: any = {
        ...
        // ✅ SYNC: totalPrice must match pricingSnapshot.finalTotal (includes borrels/merchandise)
        totalPrice: pricingSnapshot.finalTotal,
  ...
  async getAdminStats(): Promise<ApiResponse<AdminStats>> {
    ...
    try {
      ...
      const popularArrangement = Object.entries(arrangementCounts)
        .sort(([,a], [,b]) => (b as number) - (a as number))[0]?.[0] as 'standaard' | 'premium' || 'standaard';
    ...
    try {
      ...
      if (!config) {
        ...
        const migratedTypes = config.types.map(existingType => {
          ...
          if (!existingType.pricing || existingType.pricing.standaard === undefined || existingType.pricing.premium === undefined) {
            // Find default pricing for this type
            const defaultType = defaults.types.find(t => t.key === existingType.key);
            if (defaultType) {
              console.log(`✅ Adding default pricing to ${existingType.key}: standaard €${defaultType.pricing.standaard}, premium €${defaultType.pricing.premium}`);
              return {
                ...existingType,
