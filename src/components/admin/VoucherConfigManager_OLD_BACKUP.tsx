/**
 * 🆕 Voucher Configuration Manager - NIEUWE VERSIE
 * 
 ...
 * 
 * 🔥 UPDATED: Gebruikt eventTypesConfig in plaats van oude pricing.byDayType
 */

import React, { useState, useEffect } from 'react';
import { Save, Gift, Eye, EyeOff, AlertCircle, RefreshCw } from 'lucide-react';
import { useConfigStore } from '../../store/configStore';
import type { EventTypesConfig, EventTypeConfig as EventTypeConfigType } from '../../types';
...
export const VoucherConfigManager: React.FC = () => {
  const { eventTypesConfig, updateEventTypesConfig } = useConfigStore();
  ...
  useEffect(() => {
    console.log('📋 [VoucherConfig] Loading eventTypesConfig:', eventTypesConfig);
    
    // Load existing voucher settings from localStorage or create defaults
    const stored = localStorage.getItem('voucherSettings');
    if (stored) {
      try {
        setVoucherSettings(JSON.parse(stored));
      } catch (e) {
